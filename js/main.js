...
/**
    ...
    function searchProducts(query) {
        ...
        $.getJSON("products.json", function(data) {
            const filteredProducts = data.filter(product =>
                product['product-details'].toLowerCase().includes(query.toLowerCase())
            );

            displayResults(filteredProducts);
        }).fail(function() {
            console.log('Error loading products.json');
        });
    }
    ...
    function displayResults(products) {
        ...
        // Hide the main product container and show search results
        $('#product-container').hide();
        resultsContainer.show();
    
        if (products.length === 0) {
            resultsContainer.append('<div class="col-12 text-center p-t-50 p-b-50"><h3>No products found.</h3></div>');
            return;
        }
    ...
    $('.wrap-search-header').on('submit', function(e) {
        e.preventDefault();
